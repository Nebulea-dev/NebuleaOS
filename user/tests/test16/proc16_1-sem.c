...
int main(void *arg)
{
        ...
        for (i = 0; i < NBSEMS; i++)
        {
                ...
                if (sem < 0)
                        assert(!"*** Increase the semaphore capacity of your system to NBSEMS to pass this test. ***");
                sems[i] = sem;
        }
        if (screate(0) >= 0)
                assert(!"*** Decrease the semaphore capacity of your system to NBSEMS to pass this test. ***");
